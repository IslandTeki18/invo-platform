...
| Invoice viewer | `view.invo.app/invoice/{invoiceId}?token={accessToken}` | Fixed subdomain. No wildcard DNS or wildcard TLS required. Token in query string provides access control regardless of org identity. |

---

## Deferred to V2

**Org subdomain routing** (e.g., `{org}.invo.app` or `{org}.view.invo.app`) is not implemented in V1.
...
- The access token on `view.invo.app` already provides sufficient security without org-scoped URLs.
...
- `invo.app` — A/AAAA record pointing to the marketing site host.
- `admin.invo.app` — A/AAAA or CNAME pointing to the admin app host.
- `view.invo.app` — A/AAAA or CNAME pointing to the invoice viewer host.
- No wildcard DNS record required in V1.

### Deployment

- Three distinct deployment targets: marketing, admin, invoice viewer.
- Each subdomain maps 1:1 to an app. No shared-host subdomain routing logic needed in V1.
- TLS certificates are per-subdomain (three certs or a single SAN cert). No wildcard cert required.
...
- The invoice viewer (`view.invo.app`) is unauthenticated at the session level. Access is granted via a signed or opaque `token` query parameter validated server-side.
...
- Invoice emails will link to `view.invo.app/invoice/{invoiceId}?token={accessToken}`.

### Invoice Viewer Architecture

- The viewer is a standalone app served from `view.invo.app`.
